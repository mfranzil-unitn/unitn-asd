...
int solvetroppograndi() {
    ...
    is_center.resize(size_s);

    for (int i = 0; i < C; i++) {
        valori[i].resize(size_s);
        is_center[i].resize(size_s);

        for (int j = 0; j < size_s; j++) {
            int soldier_dist = min(manhattan(soldiers[j]->coord, components[i]->coord),
                                   manhattan(target, components.at(i)->coord));  // manhattan calcola in automatico la distanza soldato-componente-centro
            if (soldier_dist == manhattan(target, components.at(i)->coord)) {
                is_center[i][j] = true;
            } else {
                is_center[i][j] = false;
            }
...
int MinCostMatching(const vector<vector<int>>& cost) {
    ...
    int value = 0;
    for (int i = 0; i < C; i++) {
        if (central_soldier == -1 && !is_center[i][Lmate[i]]) {
